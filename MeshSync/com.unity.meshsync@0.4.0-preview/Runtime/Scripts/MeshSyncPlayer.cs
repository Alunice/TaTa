...
namespace Unity.MeshSync
{
    ...
    internal abstract class MeshSyncPlayer : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        internal GameObject GetExportObj() { return m_exportObj; }

        internal void SetExportObj(GameObject t) { m_exportObj = t; }
        ...
        internal bool foldExportDCC
        {
            get { return m_foldExportDCC; }
            set { m_foldExportDCC = value; }
        }

        internal bool autoUpdatePosDCC
        {
            get { return m_autoUpdatePosDCC; }
            set { m_autoUpdatePosDCC = value; }
        }

        internal bool autoHideObj
        {
            get { return m_autoHideObj; }
            set { m_autoHideObj = value; }
        }

        internal List<string> changedList
        {
        ...
        public void UpdateServerMeshTransform()
        {
            var mfs = GetExportObj().GetComponentsInChildren<MeshFilter>();
            var mymfs = GetComponentsInChildren<MeshFilter>();

            List<MeshFilter> notIncludMf = new List<MeshFilter>();
            foreach (var mymf in mymfs)
            {
                bool isIn = false;
                foreach (var mf in mfs)
                {
                    if (mf.sharedMesh != null && mymf.sharedMesh != null)
                    {
                        if (mf.sharedMesh.name == mymf.sharedMesh.name)
                        {
                            isIn = true;
                            //MeshSync只会推送DCC中有修改的内容
                            if (changedList.Contains(mf.sharedMesh.name))
                            {
                                mymf.transform.position += mf.transform.position;
                                mymf.transform.eulerAngles += mf.transform.eulerAngles;
                                mymf.transform.localScale = new Vector3(mymf.transform.localScale.x * mf.transform.lossyScale.x, mymf.transform.localScale.y * mf.transform.lossyScale.y, mymf.transform.localScale.z * mf.transform.lossyScale.z);
                            }

                            var mr = mf.gameObject.GetComponent<MeshRenderer>();
                            var mymr = mymf.gameObject.GetComponent<MeshRenderer>();

                            if (mr != null && mymr != null)
                            {
                                if (mymr.sharedMaterial != mr.sharedMaterial)
                                {
                                    mymr.sharedMaterials = mr.sharedMaterials;
                                    mymr.sharedMaterial = mr.sharedMaterial;
                                    /*
                                    var flags = GameObjectUtility.GetStaticEditorFlags(mr.gameObject);
                                    var hasgi = flags.HasFlag(StaticEditorFlags.ContributeGI);
                                    if (hasgi)
                                    {
                                        GameObjectUtility.SetStaticEditorFlags(mymr.gameObject, StaticEditorFlags.ContributeGI);
                                        mymr.receiveGI = mr.receiveGI;
                                        mymr.lightmapIndex = mr.lightmapIndex;
                                        mymr.lightmapScaleOffset = mr.lightmapScaleOffset;
                                        mymr.lightProbeUsage = mr.lightProbeUsage;
                                    }*/
                ...
                if (!isIn)
                    notIncludMf.Add(mymf);
            }
            foreach (var notin in notIncludMf)
                notin.gameObject.SetActive(false);
                //DestroyImmediate(notin.gameObject);
            notIncludMf.Clear();
        }

        public void UpdateLightMapData()
        {
            var mfs = GetExportObj().GetComponentsInChildren<MeshFilter>();
            var mymfs = GetComponentsInChildren<MeshFilter>();
            foreach (var mymf in mymfs)
        ...
        internal void AfterUpdateScene()
        {
            ...
            if (autoUpdatePosDCC && GetExportObj() != null)
                UpdateServerMeshTransform();
            //清除change列表
            m_changedList.Clear();
        ...
        [SerializeField] GameObject m_exportObj;
        [SerializeField] List<string> m_changedList = new List<string>();
        [SerializeField] bool m_hasInit = false;
        ...
        [SerializeField] bool m_foldExportDCC = true;
        [SerializeField] bool m_autoUpdatePosDCC = true;
        [SerializeField] bool m_autoHideObj = true;
